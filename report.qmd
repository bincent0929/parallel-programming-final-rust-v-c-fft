...
I am not sure exactly how the libraries that I used implemented the algorithm.
...
I believe that this `profile.release` wasn't applied because I don't have it in the `Cargo.toml` that's in the root directory of the repository. The compiler tells you that there any error when building about it. But I still want to note it in here.
...
I'm using these functions to calculate the speedups, linear ideal vs. found speedup, and what percentage of the program is parallelizable (see the repo `.qmd` for how they were used):
``` {python}
#| echo: true
...
```{python}
c_parallelizable = parallel_fraction(C_seq_v_4, 4)
rust_parallelizable = parallel_fraction(Rust_seq_v_4, 4)
print(f"C P%: {c_parallelizable:.2%}")
print(f"Rust P%: {rust_parallelizable:.2%}")
```

```{python}
min_speedup = min(C_seq_v_4, C_seq_v_8, Rust_seq_v_4, Rust_seq_v_8)
max_speedup = max(C_seq_v_4, C_seq_v_8, Rust_seq_v_4, Rust_seq_v_8)
max_parallelizability = max(c_parallelizable, rust_parallelizable)
```

## Data Analysis

Both threading methods in C and Rust did improve the speed of the base program by at least `{python} f"{min_speedup:.2f}"` times.

Rust saw the greatest speedup with a `{python} f"{max_speedup:.2f}"` times increase in speedup from $1$ thread to $8$ threads.

C was significantly faster when run sequentially than Rust at `{python} f"{c_v_rust_seq:.2f}"` times faster.

But, with threading, Rust achieved practically equivalent speeds compared to C.

Rust also had more potential to be parallelized at `{python} f"{max_parallelizability:.2%}"`.

## Conclusions

With my "vrserver" machine, under the conditions I have set for testing in this research, I can conclude that the C programming language running OpenMP using the `fftw3` threading library in `c-fft/main.c` runs faster than the Rust language running its standard threading library and `rustfft` in `rust-fft/src/main.rs` when using a single thread at a sufficient problem size. Otherwise, as thread count increases, the speed of the Rust program does follow the C program's.

### Reasoning For Results

I am unsure why the Rust program seems to be getting faster overall as thread count increases. Maybe C is getting slow when having to divide up the problem at higher thread counts that don't benefits from having the problem divided across as many threads.

I believe that the program did not see a much faster speedup with 8 threads due to a problem size that was too small for that amount of threads to see the benefits of scaling. I could run it again at a higher problem size but I did not have enough time.

I think that the problem with chunking and getting the wrong magnitude in the Rust implementation does has some issues and would need to be resolved in practical use, but I was able to demonstrate speedup with more threads and understand why it is wrong so in my case it is sufficient.

I'm not exactly sure how the C library handles the threading of the FFT. [This](https://www.fftw.org/fftw3_doc/Usage-of-Multi_002dthreaded-FFTW.html) is the page from the maintainers but I don't think it says how they decided to implement it. It could just be more efficient than my chunk approach in my Rust program.

I can parallelize the `peak_mag` finding `for` loop in C. I did parallelize it in Rust but as you can tell from the data, it wouldn't have made much a difference.
