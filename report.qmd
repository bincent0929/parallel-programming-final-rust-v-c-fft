...
min_c_over_rust = min(c_v_rust_seq, c_v_rust_4t, c_v_rust_8t)
```
...
Rust saw the greatest speedup with a `{python} f"{Rust_seq_v_8:.2f}"` times increase in speedup from $1$ thread to $8$ threads.

Nevertheless across all thread counts, C was faster than Rust by at least `{python} f"{min_c_over_rust:.2f}"` times.

Which is surprising because the C program was actually theoretically less parallelizaable at `{python} f"{parallel_fraction(C_seq_v_4, 4):.2%}"` parallelizable vs. Rust at `{python} f"{parallel_fraction(Rust_seq_v_4, 4):.2%}"`.
